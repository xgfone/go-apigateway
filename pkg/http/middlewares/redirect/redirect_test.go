...
func TestRedirect(t *testing.T) {
	m, err := Redirect("redirect", Priority, &Config{HttpToHttps: true})
	if err != nil {
		t.Fatal(err)
	}
	...
	m.Handler(func(c *runtime.Context) { c.ClientResponse.WriteHeader(204) })(c)
	if rec.Code != 204 {
		t.Errorf("expect status code 204, but got %d", rec.Code)
	}

	/// Location without Query
	m, err = Redirect("redirect", Priority, &Config{Location: "/redirect"})
	if err != nil {
		t.Fatal(err)
	}
	...
	/// Location with Query
	m, err = Redirect("redirect", Priority, &Config{Location: "/redirect", AppendQuery: true})
	if err != nil {
		t.Fatal(err)
	}
