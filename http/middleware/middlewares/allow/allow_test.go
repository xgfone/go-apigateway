...
func TestAllow(t *testing.T) {
	_, err := middleware.DefaultRegistry.Build("allow", "127.0.0.0/8")
	if err != nil {
		t.Error(err)
	}

	_, err = middleware.DefaultRegistry.Build("allow", map[string]any{"cidrs": []string{"127.0.0.0/8"}})
	if err != nil {
		t.Error(err)
	}

	_, err = middleware.DefaultRegistry.Build("allow", nil)
	if err == nil {
		t.Error("expect an error, but got nil")
	}

	_, err = middleware.DefaultRegistry.Build("allow", "127.0.0.1")
	if err == nil {
		t.Error("expect an error, but got nil")
	}

	mw, err := middleware.DefaultRegistry.Build("allow", []string{"127.0.0.0/8", "192.168.0.0/16"})
	if err != nil {
		t.Fatal(err)
	}
