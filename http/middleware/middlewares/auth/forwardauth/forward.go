...
import (
	...
	httpup "github.com/xgfone/go-apigateway/http/upstream"
	"github.com/xgfone/go-apigateway/upstream"
)

func init() {
	middleware.DefaultRegistry.Register("forwardauth", func(name string, conf any) (middleware.Middleware, error) {
		var config Config
...
// ForwardAuth returns a new middleware named "forwardauth",
// which forwards the authentication to the external server.
func ForwardAuth(config Config) (middleware.Middleware, error) {
	...
	return middleware.New("forwardauth", config, func(next core.Handler) core.Handler {
		auth := auth.with(next)
		return auth.Handle
	}), nil
}
...
func (a forwardauth) with(next core.Handler) forwardauth {
	a.next = next
	return a
}
...
func (a *forwardauth) Handle(c *core.Context) {
	...
	switch {
	case a.upid != "":
		up, ok := upstream.Manager.Get(a.upid)
		if !ok {
		...
		core.ReleaseContext(newc)

	case a.client != nil:
		resp, err = a.client.Do(req)

	case httpup.DefaultHttpClient != nil:
		resp, err = httpup.DefaultHttpClient.Do(req)
	...
	slog.Error("fail to forward auth",
		"reqid", c.RequestID(), "route", c.RouteId,
	...
	if a.degraded {
		...
		c.Abort(statuscode.ErrUnauthorized)
	}
}
